...
public class MainHook implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam param){
        ...
        if(param.packageName.equals("com.xxun.xunlauncher")){
            if(XSPUtils.getBoolean("module_xunlauncher_silence",false) || XSPUtils.getBoolean("module_xunlauncher_lowbattery",false)){
                try{
                    ...
                    if(XSPUtils.getBoolean("module_xunlauncher_lowbattery",false)) XposedHelpers.findAndHookMethod(cs, "getInt", ContentResolver.class, String.class, int.class, new XC_MethodHook() {
                        @Override
                        protected void afterHookedMethod(XC_MethodHook.MethodHookParam param) {
                            if(((String)param.args[1]).contains("lowbatterystate")) param.setResult(0);
                        }
                    });
                }catch(Exception e){
            ...
            if(XSPUtils.getBoolean("module_xunlauncher_doshutdown",false)){
                try {
                	Class<?> cs = XposedHelpers.findClass("com.xxun.xunlauncher.service.LauncherService",param.classLoader);
                    XposedHelpers.findAndHookMethod(cs,"doShutdown",String.class, new XC_MethodReplacement(){
                        @Override
                        protected Object replaceHookedMethod(MethodHookParam methodHookParam) {
                            return null;
                        }
                	...
                	AppUtils.printExceptionStack(e,"启动器 | 禁止自动开关机");
                }catch(Throwable t){
                    AppUtils.printThrowableStack(t,"启动器 | 禁止自动开关机");
                }
            }           
             
            if(XSPUtils.getBoolean("module_xunlauncher_forcestop",false)){
                try {
                	Class<?> cs = XposedHelpers.findClass("com.xxun.xunlauncher.utils.TempControlUtils",param.classLoader);
                    XposedHelpers.findAndHookMethod(cs,"RemoveAllBackGrouds",Context.class, new XC_MethodReplacement(){
                        @Override
                        protected Object replaceHookedMethod(MethodHookParam methodHookParam) {
                            return null;
                        }
                    });
                    XposedHelpers.findAndHookMethod(cs,"highTemperForceStopPackage",Context.class, new XC_MethodReplacement(){
                        @Override
                        protected Object replaceHookedMethod(MethodHookParam methodHookParam) {
                            return null;
                        }
                    });
                    XposedHelpers.findAndHookMethod(cs,"forceStopPackage",String.class ,Context.class, new XC_MethodReplacement(){
                        @Override
                        protected Object replaceHookedMethod(MethodHookParam methodHookParam) {
                            return null;
                        }
                	...
                	AppUtils.printExceptionStack(e,"启动器 | 禁止停止应用");
                }catch(Throwable t){
                    AppUtils.printThrowableStack(t,"启动器 | 禁止停止应用");
                }
            }
                        ...
                        for (int n = 0;n < packageInfos.size();n++){
                            for(int i = 0;i < hideList.length();i++){
                                if(hideList.getString(i).contains(packageInfos.get(n).packageName)){
                                    AppUtils.printLog("启动器 | 隐藏应用：" + packageInfos.get(n).packageName);
